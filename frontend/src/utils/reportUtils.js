...
const resolveApiBase = () => {
    ...
    if (!raw || raw === "undefined" || raw === "null") return "http://172.20.6.160:5000";
    if (!/^https?:\/\//i.test(raw)) raw = `http://${raw}`;
    try { const u = new URL(raw); return `${u.protocol}//${u.host}`; } catch { return "http://172.20.6.160:5000"; }
};
...
export async function downloadProReport({ sessionId, setDownloading }) {
    if (!sessionId) { alert("세션 ID가 없습니다."); return; }
    try {
    setDownloading?.(true);
    const { data } = await axios.post(`${API_BASE}/api/report/generate`, { session_id: sessionId });
